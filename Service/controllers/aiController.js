...
export const generateArticle=async (req,res)=>{
    try {
       ...
       const { userId }=req.auth(); // Replace with actual user ID from auth middleware
        const { prompt,length }=req.body;
        const plan=req.plan;
        const free_usage=req.free_usage;
        if(plan!=='premium' && free_usage>=10){
            return res.json({success:false,message:'You have exhausted your free usage limit. Please upgrade to premium plan.'})
        }
    ...
    try {
       ...
       const { userId }=req.auth(); // Replace with actual user ID from auth middleware
        ...
        if(plan!=='premium' ){
         ...
         const image_url = await cloudinary.url(public_id,{
